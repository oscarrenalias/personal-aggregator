...
class JanitorSettings(BaseSettings):
    ...
    janitor_llm_telemetry_retention_days: int = Field(30, description="Days to retain LLM call telemetry rows before purging")
    janitor_podcast_retention_days: int = Field(7, description="Days to retain podcast episodes before purging (also removes MP3 files from disk)")
