...
- **clusterer** — groups `ready` articles into threads using entity/topic overlap and LLM classification; scores and tiers threads; runs after summarize-rank. Uses a Postgres advisory lock (`pg_try_advisory_lock`) to prevent concurrent instances from racing on the same batch. Thread-membership assignment is idempotent: `assign_article_to_thread` is a no-op if the article already has a `ThreadMembership` row. The LLM classifier receives up to `CLUSTERER_MAX_CLASSIFIER_CANDIDATES` candidate threads (ranked by overlap score) rather than a single best candidate, giving it broader context for accurate thread assignment. The `dismissed` column on `Thread` is orthogonal to the lifecycle status; the clusterer never writes it, so dismissal persists across recomputation and consolidation cycles. Thread consolidation (merge pass) is incremental by default (`CLUSTERER_INCREMENTAL_MERGE=true`): only thread pairs where at least one thread gained new articles since the last consolidation are re-evaluated, skipping stable pairs. Per-pair LLM merge decisions are memoized in the `thread_merge_verdicts` table (`CLUSTERER_MERGE_VERDICT_CACHE=true`) so repeated consolidation passes do not re-call the LLM for pairs whose membership has not changed. For full (non-incremental) consolidation passes the merge-candidate working set is capped to threads updated within `CLUSTERER_MERGE_CANDIDATE_WINDOW_DAYS` (default 7 days), keeping the O(n²) pair-scoring self-join bounded as the thread table grows on a long-running Pi deployment; explicit operator recluster bypasses this window. **Thread lifecycle aging** — every poll cycle the clusterer runs two bulk SQL UPDATEs (no LLM calls, under the advisory lock): (1) `active → dormant` for threads whose `last_updated < now() - CLUSTERER_DORMANT_AGE_DAYS`; (2) `dormant → archived` for threads whose `last_updated < now() - (CLUSTERER_DORMANT_AGE_DAYS + CLUSTERER_ARCHIVE_AGE_DAYS)`. The archive cutoff is the combined age so a thread must be idle for the full dormant+archive window before reaching archived. The active→dormant pass runs first so a thread idle beyond the combined threshold jumps to archived in a single cycle. **Reactivation** — when a new article is assigned to a dormant or archived thread (via `upsert._update_thread`) or when two threads are merged (via `management.merge_threads`), the kept/updated thread's status is reset to `active` and `last_updated` is bumped. Candidate selection (`candidates.py`) does not filter on status, so dormant threads within the slow-path window (`CLUSTERER_CANDIDATE_WINDOW_DAYS_SLOW=7`) remain reachable. Because the dormant threshold (7 days) equals the slow-path window, a thread just marked dormant is still a candidate for matching on the same cycle it aged; if a new article matches it, reactivation fires and it re-enters the active working set without spawning a duplicate thread. Merge and surfacing passes filter `Thread.status == 'active'`, so only active threads are scored — dormant and archived threads are excluded from the working set, directly reducing per-cycle LLM cost.
- **web** — FastAPI + HTMX + Alpine.js read/UI surface + reader operations (mark read, save, search). Mounts the `aggregator-api` sub-app at `/api/v1` for mobile and TUI clients (see **aggregator-api** below). Served as a PWA; exposed privately over Tailscale (binds `127.0.0.1` by default). Thread dismiss/restore is handled by `POST /threads/{thread_id}/dismiss` and `POST /threads/{thread_id}/restore`; both return an `HX-Trigger: refreshThreadList` header so HTMX reloads the thread list in-place. `GET /threads` accepts a `show_dismissed=true` query param to include dismissed threads in the listing (default: excluded). `GET /threads/{thread_id}` stamps `Thread.last_viewed_at = now(UTC)` via `mark_thread_viewed` on every successful page render, which drives the `has_updates` indicator in the thread list. **`GET /threads/{thread_id}` never emits `HX-Trigger: refreshThreadList`** — opening a thread does not trigger a full list refresh (which caused a visible flash); the updated-since-visit dot on the card is cleared client-side for just the opened entry instead. **Alpine `nav` store** — registered in `shell.html` on `alpine:init` via `window.Alpine.store('nav', { current: '<initial_nav_key>' })`; `initial_nav_key` is injected by the `GET /` route so the active highlight is correct on a fresh load or hard refresh before any click. Stable key format per link type: `'smart/<view>'` for smart views (e.g. `'smart/all'`, `'smart/unread'`), `'today'` for the Today brief, `'threads'` for Threads, `'category/<name>'` for categories (set via `data-nav-key` + `$el.dataset.navKey` so URL-unsafe names work), and `'source/<id>'` for sources. **Sidebar refresh** — the sidebar `<nav id="sidebar">` listens on `hx-trigger="load, refreshSidebar from:body, every 60s"` and swaps `innerHTML` from `GET /sidebar`. The Alpine `$store.nav` and `$store.sidebar` stores are registered globally in `app.js` on `alpine:init` and therefore survive the `innerHTML` swap; no `Alpine.initTree` call is needed because the swapped fragment only contains Alpine bindings that reference these globally-registered stores. `refreshSidebar` is emitted as `HX-Trigger` on every article read/save mutation. **`VIEW_LABELS` and `view_title`** — `VIEW_LABELS` in `app.py` maps every smart-view nav key to its display name (e.g. `"smart/all" → "All"`) and is the single source of truth for smart-view labels. `_render_feed()` accepts an optional `view_title: str = ""` parameter that is forwarded to `_article_list.html`, which renders it as `<h2 class="feed-view-title">` when non-empty. Smart-view routes source `view_title` from `VIEW_LABELS[f"smart/{view}"]`; category routes use the looked-up `category.name`; source routes use `source.name`. **Mobile CSS** — the `@media (max-width: 639px)` block in `styles.css` is the sole location for mobile layout overrides; reading-text font-size overrides for `.summary-block`, `.detail-body`, and `.card-excerpt` live there and future mobile typography changes should be added in that block.
- **aggregator-api** — JSON REST API sub-app (`packages/aggregator-api/`) mounted at `/api/v1` inside the web service (`app.mount("/api/v1", api_app)` in `aggregator_web/app.py`). Provides a JSON interface for mobile and TUI clients. All paginated list responses use the `PaginatedResponse[T]` envelope: `{ "items": [...], "next_cursor": str | null }`; `next_cursor` is an opaque base64 string — pass it verbatim as the `cursor` query param to fetch the next page; `null` means no further pages. **No authentication** — the API must remain behind the network perimeter (Tailscale / `127.0.0.1`) until the Cloudflare Access auth phase lands; the write endpoints are unauthenticated and would expose read/save/dismiss mutations to any caller on the network if publicly reachable. CORS is controlled by `API_CORS_ALLOW_ORIGINS` (default `*`, acceptable only behind the perimeter).

  **Read endpoints:**
  - `GET /api/v1/articles` — list articles; params: `view` (`all`|`unread`|`important`|`saved`|`today`|`uncategorized`), `category`, `source_id`, `unread_only`, `limit`, `cursor`; backed by `queries.list_articles()`
  - `GET /api/v1/articles/search` — full-text search; params: `q`, `category`, `source_id`, `limit`, `cursor`; backed by `queries.search_articles()`
  - `GET /api/v1/articles/{id}` — single article; backed by `queries.get_article()`
  - `GET /api/v1/threads` — list threads; params: `sort` (`importance`|`recent`), `show_dismissed`, `limit`, `cursor`; backed by `queries.list_threads()`
  - `GET /api/v1/threads/{id}` — single thread (does **not** stamp `last_viewed_at` — API reads are passive and must not clear the unread indicator); returns `ThreadResponse` including `last_viewed_at` (previous view timestamp or null) and typed `deltas: List[ThreadDelta]` (heterogeneous: fact deltas + merge deltas, `extra="allow"` keeps future keys); backed by `queries.get_thread()`
  ...
  **Write endpoints (unauthenticated — perimeter-only until auth phase lands):**
  - `POST /api/v1/articles/{id}/read` — mark article read; backed by `queries.mark_read()`
  - `POST /api/v1/articles/{id}/unread` — mark article unread; backed by `queries.mark_unread()`
  - `POST /api/v1/articles/{id}/save` — save article; backed by `queries.save_article()`
  - `POST /api/v1/articles/{id}/unsave` — unsave article; backed by `queries.unsave_article()`
  - `POST /api/v1/threads/{id}/dismiss` — dismiss thread; backed by `management.set_thread_dismissed(db, id, True)`
  - `POST /api/v1/threads/{id}/restore` — restore dismissed thread; backed by `management.set_thread_dismissed(db, id, False)`
  - `POST /api/v1/threads/{id}/viewed` — record a human view; stamps `Thread.last_viewed_at = now(UTC)` via `management.mark_thread_viewed`; returns updated `ThreadResponse`; 404 if not found. **`GET /threads/{id}` stays passive** — only this explicit POST advances `last_viewed_at`.
...
- **aggregator-podcast** — scheduled daemon that generates a daily spoken-word audio episode. Three-phase LLM pipeline: (1) story selection from ready threads based on newsworthiness and continuity context from recent episodes; (2) per-segment script writing for each selected thread; (3) intro, transitions, and outro generation. Audio synthesis via a configurable TTS model (`PODCAST_TTS_MODEL`/`PODCAST_TTS_VOICE`); MP3 files written to a shared `podcasts_data` volume (`PODCAST_AUDIO_DIR`). Episodes are tracked in the `podcast_episodes` table and served by the web UI and `aggregator-api` (`GET /api/v1/podcasts`, `GET /api/v1/podcasts/{id}`, `GET /api/v1/podcasts/latest`, `GET /api/v1/podcasts/by-date/{YYYY-MM-DD}`, `GET /api/v1/podcasts/{id}/audio`, `GET /api/v1/podcasts/{id}/script`). Runs once per day at `PODCAST_GENERATION_HOUR`; old episodes are pruned by the janitor after `JANITOR_PODCAST_RETENTION_DAYS`.
...
| `make openapi` | Regenerate `docs/openapi.json` from code — run this after any API route change and commit the result |
...
| `CLUSTERER_DORMANT_AGE_DAYS` | `7` | Days since `last_updated` before a thread transitions active→dormant (aging pass runs every cycle) |
| `CLUSTERER_ARCHIVE_AGE_DAYS` | `30` | Additional days beyond `CLUSTERER_DORMANT_AGE_DAYS` before dormant→archived (total idle time = dormant + archive days) |
...
| `CLUSTERER_MERGE_CANDIDATE_WINDOW_DAYS` | `7` | Maximum age in days for threads included in the O(n²) merge-candidate self-join during full (non-incremental) consolidation passes; threads older than this are excluded, keeping the pair-scoring cost bounded as the thread table grows. Explicit operator recluster bypasses this window to always evaluate all pairs. |
